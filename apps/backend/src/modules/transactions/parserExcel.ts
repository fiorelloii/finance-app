...
export const parseExcel = (filePathArg?: string) => {
  ...
  function parseDateString(v: any) {
    if (!v && v !== 0) return '';
  ...
  const filtered = normalized.filter((r) => {
    ...
    if (!r.descrizione_operazione || r.descrizione_operazione.length < 3) return false;

    // require at least one of uscita/entrata to be non-zero
    if ((!r.uscita || Number(r.uscita) === 0) && (!r.entrata || Number(r.entrata) === 0)) return false;

    // filter out transfer categories
    if (isTransfer(r.categoria)) return false;
  ...
  for (const r of filtered) {
    if (!r.categoria || r.categoria === '') r.categoria = 'Altro';
  }
  ...
  const aggregatedSpese: Array<{ label: string; value: number }> = [];
  const aggregatedEntrate: Array<{ label: string; value: number }> = [];
  ...
  while (true) {
    ...
    if (!category) break; // Stop when category column is empty
