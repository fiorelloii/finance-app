...
exports.aggregateByCategory = exports.filterByPeriod = void 0;
...
const filterByPeriod = (data, filter, referenceDate, endDate) => {
    const now = referenceDate || (0, dayjs_1.default)();
    return data.filter((item) => {
        const raw = item.Date ?? item.data ?? item.Data ?? item.date;
        const date = (0, dayjs_1.default)(raw);
        switch (filter) {
            case 'day':
...
const aggregateByCategory = (data) => {
    ...
    data.forEach((item) => {
        ...
        const raw = (item.uscita && Number(item.uscita) !== 0 ? item.uscita : null) ??
